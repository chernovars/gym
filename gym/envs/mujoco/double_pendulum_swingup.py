...
class DoublePendulumSwingUpEnv(mujoco_env.MujocoEnv, utils.EzPickle):

    def __init__(self):
        mujoco_env.MujocoEnv.__init__(self, 'double_pendulum_swingup.xml', 5)
    ...
    def _step(self, action):
        ...
        vel_penalty = 0
        alive_bonus = 10
        dist_penalty = -0.036 * (y ** 2) + 1.8
        wall_penalty = 5 * (ob[0] ** 2)
        r = (alive_bonus - dist_penalty - wall_penalty)
        done = bool(abs(ob[0]) >= 0.99)
        return ob, r, done, {}
